...
void ScitosDrive::initialize() {
  ...
  //TODO: msg type? topic?
  move_base_pub_ = robot_->getRosNode().advertise<geometry_msgs::PoseStamped>("/move_base", 20);
  ...
  //TODO data type? channel? http://www.mira-project.org/MIRA-doc/domains/robot/SCITOSConfigs/index.html
  robot_->getMiraAuthority().subscribe<mira::Point2f>("/navigation/Pilot",
  					      &ScitosDrive::move_base_callback, this);
...
//TODO: channel? accessing data?

void ScitosDrive::move_base_callback(mira::ChannelRead<mira::Point2f> data){
	mira::navigation::TaskPtr task(new mira::navigation::Task());
	task->addSubTask(mira::navigation::SubTaskPtr(new mira::navigation::PositionTask(mira::Point2f(data->value().x(), data->value().y()), 0.3f, 0.3f)));
